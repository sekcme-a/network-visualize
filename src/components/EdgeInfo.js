  ...
  return(
    <div className={style.container}>
      ...
      <div style={{display:"flex", alignItems:"center", margin: "10px 0 0 0" }}>
        <h3>{sourceNode?.label}</h3>
        <p style={{fontSize:"13px", fontStyle:"italic"}}>{`[${sourceNode?.identifier?.slice(5)}]`}</p>
      </div>
      <p style={{fontSize:"12px"}}>{sourceNode?.domain_summary_url}</p>
      <div style={{display:"flex", alignItems:"center", margin: "10px 0 0 0" }}>
        <h3>{targetNode?.label}</h3>
        <p style={{fontSize:"13px", fontStyle:"italic"}}>{`[${targetNode?.identifier?.slice(5)}]`}</p>
      </div>
